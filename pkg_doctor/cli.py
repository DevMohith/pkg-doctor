...
from pkg_doctor import __version__
from pkg_doctor.scanner import discover_projects, PackageRef, ProjectManifest
from pkg_doctor import osv_client
from pkg_doctor.secrets_scan import find_secrets
from pkg_doctor.remediation import run_remediation
...
BANNER = f"{PURPLE}{BOLD}pkg-doctor{RESET} {CYAN}v{__version__} - local supply-chain & exposed-secret triage{RESET}"


@click.command()
@click.version_option(version=__version__, prog_name="pkg-doctor", message="%(prog)s %(version)s")
@click.option(
    "--path", "paths",
    multiple=True,
    required=True,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Also audit pkg-doctor's own installed dependencies.",
)
@click.option(
...
def cli(paths, include_self, skip_secrets):
    """
    pkg-doctor - scan local projects for known-vulnerable or malicious dependencies (via OSV.dev),
    ...
    """
    click.echo(BANNER)
    click.echo(f"{PURPLE}[pkg-doctor]{RESET} Scanning {len(paths)} path(s)...")
    ...
    if include_self:
        ...
        projects.append(ProjectManifest(project_dir=Path("<pkg-doctor (self)>"), packages=self_pkgs))
